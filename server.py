...
def evaluation(config, logger=None):
    ...
    logger.info(f"Finished words creation in {time.time() - tic:.3f}s")
    ...
    for j, batchword in enumerate(Words):
      for i, lstVwidx in enumerate(test_loader):
        ...
        print("DEBUG:", j, i)
        logger.info(f"Finished transform batch in {time.time() - tic:.3f}s")
        tic = time.time()
        ...
        logger.info(f"Finished phoneme tensor creation in {time.time() - tic:.3f}s")
        ...
        for k in range(0,len(target)):
            ...
            for m in range(0,lens[k]):
              ...
              preds_loc = model(vis_feat_lens=lens_loc,p_lengths=None, phonemes=phonemeTensor[:,k].unsqueeze(1).detach(), graphemes=graphemeTensor[:-1][:,k].unsqueeze(1).detach(), vis_feats=input_var_chunck, use_BE_localiser=use_BE_localiser, epoch=74, config=config)                  
    ...
    #print("Demo output file created at data/lipspeak/demo.png")
            ...
            #####EH inferece_code_block#####
            results[batchword] = logits
            logger.info(f"Finished model invocation in {time.time() - tic:.3f}s")
            #####EH inferece_code_block#####
